...
設計原則:
  * 引擎 lazy 載入 — 首次呼叫才 import 引擎依賴，不拖慢 app 啟動。
  * 純記憶體處理 — 輸入 bytes、輸出文字行，圖片不落盤。
  * 輸出每行含信心度 (score)，由呼叫端決定如何呈現。

引擎選擇（環境變數 BETGUARD_OCR_ENGINE）:
  * 預設 (rapidocr) — RapidOCR / ONNX Runtime，本機快速、輕量。
  * glm — GLM-OCR 0.9B 本地 transformers 引擎（手寫牌單效果較好），
    見 ocr_glm.py。
"""
...
def _use_glm_engine() -> bool:
    """是否使用 GLM-OCR 引擎（BETGUARD_OCR_ENGINE=glm）。"""
    return os.environ.get("BETGUARD_OCR_ENGINE", "").strip().lower() == "glm"
...
def extract_text(image_bytes: bytes) -> list[dict[str, Any]]:
    ...
    if _use_glm_engine():
        from .ocr_glm import extract_text as _glm_extract

        return _glm_extract(image_bytes)
